...
export const Route = createFileRoute("/_authed/admin/custom-widgets/")({
...
const columns: ColumnDef<SafeCustomWidget>[] = [
  {
    ...
    header: "Name",
    cell: ({ row }) => (
      <Link to="/admin/custom-widgets/$widgetId" params={{ widgetId: row.original.id }}>
        {row.original.name}
      </Link>
    ),
  },
...
function CreateCustomWidget() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      navigate({ to: "/admin/custom-widgets/$widgetId", params: { widgetId: created.id } });
    } catch (err) {
